...
The app does not calculate payable subscription occurrences directly from `start_date` during budget views or payments. `start_date` is used when creating future `subscription_dates`, but charge display, manual payment, and autopay are based on persisted `subscription_dates`.
...
3. `subscription_dates` rows are created from `start_date` through the current active month.
...
4. `subscription_dates` are ensured through the current active month.
...
Budget subscription lists load persisted `subscription_dates` within the requested date range.
...
2. The API validates that the occurrence date exists in `subscription_dates`.
...
- have a `subscription_dates` row for the current date
...
2. Creates `subscription_dates`.
3. Backfills `subscription_amounts` from the old `subscriptions.amount` column using `subscriptions.start_date`.
4. Backfills `subscription_dates` from existing subscription recurrence rules through the current month.
